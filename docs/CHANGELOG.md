...
- “设置 → 账号设置 → 日常与活动”新增雨落成诗二级卡片，可按账号开启自动购买天气采集瓶、自动采集好友雷雨、自动使用雷雨召唤瓶、快速刷取雷电变异作物和自动解锁气象研究。
- 新增每日雷电变异作物冲刺策略：任务未达到 `10/10` 时按需召唤雷雨，并在天气剩余时间足以覆盖成熟周期时优先种植三品、单季且 100 分钟成熟的鲜姜；不铲除现有作物，完成后自动恢复账号原种植策略。
- “快速刷取雷电变异作物”与持续“使用雷雨召唤瓶”模式互斥，开启冲刺时自动关闭持续召唤；收获后立即刷新服务端任务进度，达到每日上限后停止续瓶和冲刺种植。
- 使坏瓶适配支持，当前并未测试效果。
- 雨落成诗自动开关跟随活动有效期管理，活动未开始或结束后自动关闭，前端二级卡片与活动入口同步置灰或隐藏。
- 新增雨落成诗协议解析、活动边界、召唤瓶请求、闪电变异识别、三品作物最快候选和开关互斥测试；后端完整测试集共 211 项全部通过，并通过前端类型检查和生产构建。
